...
- they don't need to run frequently
...
| Optimization | Takes advantage of query optimization capabilities | It does not reuse the same source that has already been executed by a model and run it again. |
| Run logic | Processes only changed data for large data sets (incremental run) | Processes all data every time it is run |

**Figure 2** - Comparison between dbt and Alteryx
...
1. **Start by refactoring smaller Alteryx workflows and then move on to more complex ones**
...
1. **Identify the source models and refactor the Alteryx from left to right**

The first step is to validate all data sources and create one common table expression (CTE) for each source referenced in the specific Alteryx workflow being refactored, so that it is easy to reuse them throughout the model.
...
1. **Use the audit_helper package to audit refactored data models**
...
1. **Duplicate reports and connect them to the dbt refactored models**
...
The conversion proved to be of great value to the client due to 3 main aspects of the new dbt-based data stack, which were observed by both teams:
...
- Improved workflow visibility: dbt’s support for documentation and testing, associated with the dbt Cloud interface, allows for great visibility of the workflow’s lineage execution, accelerating errors and data inconsistencies identification and troubleshooting. More than once, our team was able to identify the impact of one column’s logic alteration in downstream models much earlier than these Alteryx models;
...
>
>
> [https://docs.getdbt.com/blog/audit-helper-for-migration](https://docs.getdbt.com/blog/audit-helper-for-migration)
>
> [https://docs.getdbt.com/guides/migration/tools/refactoring-legacy-sql](https://docs.getdbt.com/guides/migration/tools/refactoring-legacy-sql)
>